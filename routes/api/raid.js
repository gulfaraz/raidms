module.exports = function (util, router, Raid, auth) {
    router.route("/")
        .post(auth.passport.authenticate("bearer"), function (req, res) {
            if(req.authInfo.scope === "edit" || req.authInfo.scope === "manage") {
                ...
                if(req.user.role === "new") {
                    message = "Please check your registered mail to complete the registration";
                }
                res.json({
                    "success" : false,
                    "message" : message
        ...
        .get(function (req, res) {
            var tableState = JSON.parse(req.query.tableState);
            if(!tableState) {
                tableState = {
                    "sort" : {
            ...
            Raid.find({
                ...
                .exec(function (err, raid) {
                    if(err) {
                        ...
                        res.json({
                            "success" : true,
                            "data" : raid.slice(start, start + number),
                            "numberOfPages" : Math.ceil(raid.length/number)
                        });
                    }
                });
        });
                            ...
                            if(req.body.action === "admit") {
                                if(req.body.player) {
                                    if(raid.queue.indexOf(req.body.player) >= 0 &&
                                        raid.players.indexOf(req.body.player) < 0) {
                                        Raid.find({ "players" : req.body.player })
                                            .exec(function (err, exists) {
                                                if(err) {
                                                    res.json({
                                                        "success" : false,
